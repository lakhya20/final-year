...
import { FileTextIcon, CheckCircleIcon, BrainCircuitIcon, TrendingUpIcon } from "lucide-react";
...
export default function DashboardPage() {
  ...
  const bestModel = metrics.reduce(
    (best, m) => (m.Accuracy > (best?.Accuracy ?? 0) ? m : best),
    metrics[0]
  );
    ...
    <PageShell>
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          Overview of the inflation research classification system
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <StatCard
          icon={<FileTextIcon className="size-5 text-blue-500" />}
          label="Total Papers"
        ...
        <StatCard
          ...
          value={loading ? "—" : `${stats?.relevant.toLocaleString()} (${stats?.relevant_pct}%)`}
        />
        <StatCard
          icon={<BrainCircuitIcon className="size-5 text-purple-500" />}
          label="Models Trained"
          value={loading ? "—" : metrics.length.toString()}
        />
        <StatCard
          icon={<TrendingUpIcon className="size-5 text-orange-500" />}
          label="Best Accuracy"
          value={loading ? "—" : bestModel ? `${(bestModel.Accuracy * 100).toFixed(1)}%` : "—"}
          sub={bestModel?.Model}
        />
      </div>
